...
class LoggedInData extends Component{
    ...
    // This function is submitted when a user wants to add an exotic style (ignore the underscore) item to the user's entry in the database
    ExoticStyleNameSubmit=(e)=>{
    ...
    render(){
        ...
        if(!this.props.loggedInUserInfo.loggedIn){
            return(<div>
                <h1>NOT LOGGED IN!!!</h1>
            </div>);
        }
        // if the user IS logged in. Show the form to allow them to create a new todoItem
            ...
            return (
                <div>
                    <h1>Welcome {this.props.loggedInUserInfo.username}</h1>
                    {/*Form for entering a new todoItem. Once you submit the form it runs ToDoItemSubmit*/}
                    <form onSubmit={this.ExoticStyleNameSubmit}>
                        <p>
                            <label htmlFor={"exoticStyleName"}>Enter an additional exoticStyle:</label>
